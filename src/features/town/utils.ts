...
export const getNextTurnResources = (town: Town): Resources => {
    ...
    let nextTurnResources = mergeResources(town.resources, BASE_RESOURCE_GATHER_RATES);

    // Add building modifiers
    town.buildings.forEach((building) => {
        nextTurnResources = mergeResources(nextTurnResources, building.resources);
    });

    // Add villager modifiers
    town.villagers.forEach((villager) => {
        nextTurnResources = mergeResources(nextTurnResources, villager.resources);
    });
